...
function player(state, color) {
  return state.players.find((p) => p.color === color) ?? null
}

export function chefOf(state) {
  return player(state, state.chef)
}

export function heirOf(state) {
  return player(state, state.nextChef)
}

export function aceEaters(log) {
  const seen = []
  for (const d of log ?? []) {
...
function say(state, message, extra = {}) {
  return { ...state, message, ...extra }
}

function mapPlayer(state, color, fn) {
  return state.players.map((p) => (p.color === color ? fn(p) : p))
}

export function isPaquetGame(game) {
  return Boolean(game && game.table === 'paquet')
}
...
export function createPaquetFromPlayers(id, players, stake, paid = true, chef = null) {
  const table = {
    table: 'paquet',
    id,
    hand: 1,
...
export function resolveElect(state) {
  if (!state || state.phase !== 'elect') return state
...
export function freePackets(state) {
  return state.packets.filter((p) => !p.takenBy).map((p) => p.id)
}

export function pickPacket(state, color, packetId) {
...
export function betOptions(state, color) {
  const who = player(state, color)
  if (!who) return []
  const min = minBet(state)
  const max = who.coins
...
export function placeBet(state, color, amount) {
  if (!state || state.phase !== 'bet' || state.actor !== color || color === state.chef) return state
  const who = player(state, color)
...
export function peekChef(state, color) {
  if (!state || state.phase !== 'peek' || state.chef !== color) return state
  const chef = chefOf(state)
...
export function coverCurrent(state, color) {
  if (!state || state.phase !== 'cover' || state.chef !== color || !state.challenger) return state
  const chef = chefOf(state)
  ...
  if (cover <= 0) {
    ...
    if (!following) {
      return say({ ...refunded, phase: 'hand', actor: null }, `${chef.name} ne peut plus suivre. Coup fini.`)
    }
    return say(
      {
        ...refunded,
...
export function resolveDuel(state) {
  if (!state || state.phase !== 'duel' || !state.chef || !state.challenger) return state
  const chef = chefOf(state)
  const vs = player(state, state.challenger)
  ...
  const lastDuel = {
    vs: vs.color,
    chef: chef.color,
    winner: playerWins ? vs.color : chef.color,
    bet,
    payout,
  ...
  const players = state.players.map((p) => {
    if (p.color === vs.color) return { ...p, coins: p.coins + (playerWins ? payout : 0), settled: true, bet: 0 }
  ...
  const next = { ...state, lastDuel, log, pot, players, nextChef, claimants }
  const following = nextOpenBet(next, vs.color)
  const houseNote = house ? ` Maison ${formatLudo(house)}.` : ''
  const aceNote =
    playerWins && vsPack.card.rank === 14
      ? claimants.length > 1
...
function closeCoup(state, message) {
...
function beginRunoff(state, prefix) {
...
function resolveRunoff(state) {
  const claimants = state.claimants ?? []
  let best = null
  for (const color of claimants) {
    const who = player(state, color)
    const pack = state.packets.find((p) => p.id === who?.packet)
    if (!who || !pack?.card) continue
    if (!best || cardPower(pack.card) > cardPower(best.card)) best = { ...who, card: pack.card }
  }
...
export function offerChef(state, color, amount) {
  if (!state || state.phase !== 'hand' || state.chef !== color) return state
  const chef = chefOf(state)
  const value = Math.floor(Number(amount))
...
export function cancelOffer(state, color) {
  if (!state || state.phase !== 'hand' || state.chef !== color || !state.offer) return state
  const chef = chefOf(state)
  return say({ ...state, offer: null }, `${chef?.name ?? 'Chef'} garde le chef.`)
}

export function buyChef(state, color) {
  if (!state || state.phase !== 'hand' || !state.offer || color === state.chef) return state
...
export function stepAuto(state) {
  if (!state) return state
  if (state.phase === 'elect') return resolveElect(state)
  if (state.phase === 'named') return dealPackets(state)
  if (state.phase === 'claim') return say({ ...state, phase: 'hand', actor: null, offer: null }, state.message)
  ...
  if (state.phase === 'bet') {
    ...
    if (!amount) {
      const skip = aroundChef(state).find(
        (p) => p.bet === 0 && p.packet != null && p.color !== who.color && p.coins >= minBet(state),
      )
...
export function botPick(state) {
  return stepAuto(state)
}

export function allPicked(state) {
  return Boolean(state?.players?.every((p) => p.packet != null))
}

function canSeePacket(state, viewer, packet) {
