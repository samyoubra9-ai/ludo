...
export function sha256Bytes(input: Uint8Array): Uint8Array {
  ...
  const withBit = input.length + 1
  const pad = (64 - ((withBit + 8) % 64)) % 64
  const packed = new Uint8Array(withBit + pad + 8)
  packed.set(input)
  packed[input.length] = 0x80
  const view = new DataView(packed.buffer)
  view.setUint32(packed.length - 4, bitLen)
  ...
  for (let offset = 0; offset < packed.length; offset += 64) {
